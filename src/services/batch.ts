 ...
 */
export function getBatchStatusFromKey(status: ClientBatchStatus.Status) {
 ...
 */
export async function waitForBatchCommit(batchStatusLink: string) {
  ...
  switch (getBatchStatusFromKey(status)) {
    ...
    default:
      ...
      waitForBatchCommit(batchStatusLink);
      break;
  }
 ...
 */
export async function submitBatch(batchListBytes: Uint8Array) {
  let res;
  ...
  await waitForBatchCommit(batchStatusLink);
}
